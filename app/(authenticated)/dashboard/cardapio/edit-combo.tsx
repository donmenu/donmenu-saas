...
import type { Combo, MenuItem } from '../../../../types/combo';
...
export default function EditCombo({ isOpen, onClose, combo, onSuccess }: EditComboProps) {
  ...
  const populateForm = () => {
    ...
    setDiscount(combo.discount?.toString() || '');
    setImageUrl(combo.image_url || '');
    setValidFrom(combo.valid_from ? new Date(combo.valid_from).toISOString().slice(0, 16) : '');
    setValidTo(combo.valid_to ? new Date(combo.valid_to).toISOString().slice(0, 16) : '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!combo || !name.trim() || !price || items.length === 0) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Erro ao atualizar combo');
      }

      onSuccess();
    } catch (error: any) {
      console.error('Erro ao atualizar combo:', error);
      setError(error.message || 'Erro ao atualizar combo');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          ...
          <div className="flex justify-between items-center mb-6">
            <Title className="text-2xl font-bold">Editar Combo</Title>
            <Button
              onClick={onClose}
              variant="secondary"
              size="sm"
            ...
            <Card>
              ...
              <div className="mt-4">
                <label className="flex items-center">
                  ...
                  />
                  <Text className="ml-2">Combo ativo</Text>
                </label>
              </div>
            </Card>
